...
{
	int nb_tours = myRandomMinMax(1,amplitude);
	int v1=0;
	int v2=0;
	int tmp;
	int i;
	*cheminY=cheminX;
	for(i=0;i<nb_tours;i++){
	      v1 = myRandomMinMax(0,cheminY->nbVilles -1);
	      v2 = myRandomMinMax(0,cheminY->nbVilles -1);
	      while(v1==v2){
	      v2 = myRandomMinMax(0,cheminY->nbVilles -1);
	      }	
	      tmp = cheminY ->parcours[v1];
	      cheminY->parcours[v1] = cheminY->parcours[v2];
	      cheminY->parcours[v2] = tmp;
	}
...
double g(void) {
  ...
  #else
    return T-alpha;
  #endif          /* on decroit le temperature en utilisant alpha */
}
