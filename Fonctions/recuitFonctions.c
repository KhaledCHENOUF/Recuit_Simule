...
void transformation(void){
  y = x + amplitude * (myRandom01()-0.5) ;                     /* y est au voisinage de x suivant amplitude */
}
...
double g(void) {
 ...
 return T*alpha ;
#else
 return T-alpha;            /* on decroit le température en utilisant alpha */
#endif
}
    ...
    while ( T > Tf ) {                            /* 1er critere d'arret */
       ...
       for(rep=0; rep<MaxRepetitions; rep++){ /* 2eme critere d'arret */
          ...
          if ( Df < 0 ) {                   /* Descente !!              */
                ...
                fx = fy ;                                
                if( fxopt > fx ){                /* Mise a jour optimum ?    */
                    xopt  = x ;
                    fxopt = fx ;
#if VISUMEILLEUR==1
